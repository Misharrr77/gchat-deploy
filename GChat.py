...
def create_permanent_tunnel_with_token(cloudflared_cmd, port):
    """Создает постоянный туннель через Cloudflare Tunnel Token (основной метод)"""
    print(" * 🔐 Основной метод: создание туннеля через Cloudflare Tunnel Token...")
    ...
    if not tunnel_token:
        print(" * ⚠️ CLOUDFLARE_TUNNEL_TOKEN не найден.")
        return None
    ...
    try:
        ...
        print(f" * 🔍 Проверка существующего туннеля '{tunnel_name}'...")
        list_result = subprocess.run(
        ...
        if not tunnel_exists:
            print(f" * 🏗️ Создание именованного туннеля '{tunnel_name}' через токен...")
            # Создаем туннель через токен
            ...
            if create_result.returncode != 0:
                if "already exists" not in create_result.stderr.lower() and "already exists" not in create_result.stdout.lower():
                    print(f" * ⚠️ Ошибка создания туннеля: {create_result.stderr or create_result.stdout}")
                    return None
                else:
                    print(f" * ✅ Туннель '{tunnel_name}' уже существует")
            else:
                print(f" * ✅ Туннель '{tunnel_name}' успешно создан")
        else:
            print(f" * ✅ Туннель '{tunnel_name}' найден")
        
        # Сохраняем credentials из токена
        try:
            import base64
            import json
            # Декодируем токен и создаем credentials файл
            token_parts = tunnel_token.split('.')
            if len(token_parts) >= 2:
                # Пытаемся декодировать payload токена
                try:
                    ...
                    if account_id and tunnel_id:
                        ...
                        print(f" * ✅ Credentials сохранены")
                except:
                    # Если не получилось декодировать, создаем простой credentials файл
                    credentials = {"TunnelToken": tunnel_token}
                    with open(credentials_file, 'w') as f:
                        json.dump(credentials, f)
        except Exception as cred_err:
            print(f" * ⚠️ Предупреждение при сохранении credentials: {cred_err}")
...
"""
        ...
        print(f" * 🚀 Запуск постоянного туннеля...")
        ...
        if tunnel_process.poll() is None:
            ...
            print(f" * 🔗 Подключение: https://{tunnel_hostname} -> http://127.0.0.1:{port}")
            return tunnel_process
        else:
            print(f" * ⚠️ Туннель завершился с кодом: {tunnel_process.poll()}")
            return None
            
    except subprocess.TimeoutExpired:
        print(f" * ⚠️ Таймаут при создании туннеля")
        return None
    except Exception as e:
        print(f" * ⚠️ Ошибка создания туннеля через токен: {e}")
        import traceback
        traceback.print_exc()
        return None
...
def setup_cloudflare_tunnel(port):
    ...
    # ОСНОВНОЙ МЕТОД: Пробуем создать туннель через токен (приоритет #1)
    tunnel_token = os.environ.get("CLOUDFLARE_TUNNEL_TOKEN", "eyJhIjoiMzIxYjI5MDQ5OTNkMzEwNDI2ZTdhMTViNmViYTI3MjciLCJ0IjoiMTU1Mjc2NDktNDExNi00ZWQzLThhZGItNjI2NTMyMjQ2NzA1IiwicyI6Ik0yUmtZV0l5Wm1RdE0yVTJOQzAwT0dJMExXRm1ZV1V0WW1NeFlqWTFOVGM1WVdNNSJ9")
    if tunnel_token:
...
def run_cloudflare_tunnel(port):
    """Запускает Cloudflare Tunnel в отдельном потоке"""
    def tunnel_thread():
